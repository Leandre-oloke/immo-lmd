// lib/views/profile/page_profil.dart
...
class _PageProfilState extends State<PageProfil> {
  ...
  void _loadStats() {
    ...
    if (currentUser.role == 'owner') {
      context.read<OwnerViewModel>().loadOwnerLogements(currentUser.id);
    } else if (currentUser.role == 'admin') {
      context.read<AdminViewModel>().loadStatistics();
    }
    
    // Charger tous les logements pour tous les utilisateurs
  ...
  Widget build(BuildContext context) {
    ...
    if (currentUser == null) {
      return Scaffold(
        appBar: AppBar(
          ...
          // ✅ Bouton retour automatique si on vient d'une autre page
        ),
    ...
    return Scaffold(
      appBar: AppBar(
        // ✅ Le bouton retour est automatiquement ajouté par Flutter
        // si la page a été pushée sur la pile de navigation
      ...
      body: WillPopScope(
        ...
        onWillPop: () async {
          if (_isEditing) {
            // Si en mode édition, annuler l'édition au lieu de quitter
            _cancelEditing();
            return false; // Ne pas quitter la page
          }
          return true; // Permettre le retour normal
        },
  ...
  Widget _buildStatsSection(dynamic currentUser) {
    ...
    if (currentUser.role == 'owner') {
      ...
      final totalLogements = ownerViewModel.myLogements.length;
      final disponibles = ownerViewModel.availableLogements.length;
      final occupes = ownerViewModel.occupiedLogements.length;
      final revenuEstime = ownerViewModel.ownerStats['estimatedRevenue'] ?? 0.0;
      
      stats.addAll([
        {
          'label': 'Logements publiés',
          'value': totalLogements.toString(),
          'icon': Icons.home,
          'color': Colors.blue,
        },
        {
        ...
        {
          'label': 'Occupés',
          'value': occupes.toString(),
          'icon': Icons.bed,
          'color': Colors.orange,
        },
        {
          'label': 'Revenu estimé',
          'value': '${revenuEstime.toStringAsFixed(0)}€',
          'icon': Icons.euro,
          'color': Colors.purple,
        },
      ]);
      ...
      final favorisCount = logementViewModel.favoris.length;
      ...
      stats.addAll([
        {
          'label': 'Utilisateurs',
          'value': adminViewModel.totalUsers.toString(),
          'icon': Icons.people,
          'color': Colors.blue,
        },
        {
          'label': 'Logements',
          'value': adminViewModel.totalLogements.toString(),
          'icon': Icons.apartment,
          'color': Colors.green,
        },
        {
          'label': 'Propriétaires',
          'value': adminViewModel.totalOwners.toString(),
          'icon': Icons.business,
          'color': Colors.orange,
        },
        {
          'label': 'Actifs',
          'value': adminViewModel.activeLogements.toString(),
          'icon': Icons.verified,
          'color': Colors.purple,
        },
      ]);
...
// StatefulWidget pour permettre l'édition des informations
